...
if __name__ == "__main__":
    ...
    if db.connect():
        ...
        try:
            ...
            # Authentification
            user, message = db.authenticate_user('john.doe@example.com', 'SecurePassword123!')
            if user:
                print(f"✅ Authentification réussie pour {user['first_name']} {user['last_name']}")
                
                # Ajout de compétences
                db.add_user_skill(user_id, 'Python', 'Advanced', 5, True)
                db.add_user_skill(user_id, 'JavaScript', 'Intermediate', 3)
